...
/**
 * The default encryptor implementation for Android and J2SE.
 */
public class DefaultEncryptor {
  ...
  public DefaultEncryptor(EncryptorConfig cfg, final byte[] key)
    ...
    if (null == key || key.length != cfg.getKeySize()) {
  ...
  /**
   * Create an encryptor using default algorithms, i.e. "AES/CBC/PKCS5Padding"
   * @param key encryption key
   * @throws InvalidKeyException
   * @throws NoSuchAlgorithmException
   * @throws NoSuchPaddingException
  ...
  public String encodeToString(final byte[] plain) {
    ...
    try {
      ...
      String result = Base64.encodeBytes(bResult);
      return result;
    } catch (Exception e) {
  ...
  public byte[] decodeFromString(final CharSequence cipherTxt) {
    ...
    try {
      final byte[] txt = Base64.decode(cipherTxt.toString());
