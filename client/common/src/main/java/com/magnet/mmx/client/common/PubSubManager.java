...
public class PubSubManager {
  ...
  public String publish(MMXPersonalTopic topic, MMXPayload payload,
    ...
    String realTopic = (topic.getUserId() == null) ? makeAppTopic(topicName) :
        makeUserTopic(topic.getUserId(), topicName);
    boolean redo;
    do {
      try {
        redo = false;
  ...
  public boolean retract(MMXTopic topic, List<String> itemIds)
    ...
    String topicPath = TopicHelper.normalizePath(topic.getName());
    RetractRequest rqt = new RetractRequest(topic.getUserId(), topicPath, itemIds);
  ...
  public List<MMXMessage> getItems(MMXTopic topic, FetchOptions options)
    ...
    String topicPath = TopicHelper.normalizePath(topic.getName());
    FetchRequest rqt = new FetchRequest(topic.getUserId(), topicPath, options);
...
//            ", date="+item.getCreationDate()+", payload="+item.getPayloadXml());
  ...
  public String subscribe(MMXTopic topic, boolean thisDeviceOnly)
    ...
    SubscribeRequest rqt = new SubscribeRequest(topic.getUserId(), topicPath, devId);
  ...
  public boolean unsubscribe(MMXTopic topic, String subscriptionId)
    ...
    String topicPath = TopicHelper.normalizePath(topic.getName());
    UnsubscribeRequest rqt = new UnsubscribeRequest(topic.getUserId(),
  ...
  public List<MMXSubscription> listAllSubscriptions() throws MMXException {
    return listSubscriptions(null, ListType.global);
//    return listSubscriptions(null, ListType.both);
  }
...
//    return listSubscriptions(topic, ListType.both);
    return listSubscriptions(topicName, ListType.global);
  }
  ...
  public List<MMXTopicInfo> listTopics() throws MMXException {
    return listTopics(null, ListType.global, false);
//    return listTopics(null, ListType.both, true);
  }
...
//  }
  ...
  // Make a personal topic of a user as "/appId/userID/topic".  The userID
  // is always lower case.
  private String makeUserTopic(String userId, String topic) {
    if (topic.charAt(0) == '/') {
      return mAppPrefix+userId.toLowerCase()+topic.toLowerCase();
    } else {
      return mAppPrefix+userId.toLowerCase()+'/'+topic.toLowerCase();
    }
  }
  ...
  private MMXStatus doTags(PubSubCommand cmd, MMXTopic topic,
    ...
    TopicTags rqt = new TopicTags(topic.getUserId(), topic.getName(), tags);
